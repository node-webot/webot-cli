...
exports.info2xml = _.template([
  '<xml>',
    ...
    '<% }else if(type=="event"){  %>',
      ...
      '<% } ; if (data.merchantId) {  %>',
        '<MerchantId><![CDATA[<%= data.merchantId %>]]></MerchantId> ',
      '<% } ; if (data.isPass) {  %>',
        '<IsPass><![CDATA[<%= data.isPass %>]]></IsPass> ',
      '<% } ; if (data.reason) {  %>',
        '<Reason><![CDATA[<%= data.reason %>]]></Reason> ',
      '<% } ; if (data.uniqId) {  %>',
        '<UniqId><![CDATA[<%= data.uniqId %>]]></UniqId> ',
      '<% } ; if (data.poiId) {  %>',
        '<PoiId><![CDATA[<%= data.poiId %>]]></PoiId> ',
      '<% } ; if (data.result) {  %>',
        '<Result><![CDATA[<%= data.result %>]]></Result> ',
      '<% } ; if (data.msg) {  %>',
        '<msg><![CDATA[<%= data.msg %>]]></msg> ',
